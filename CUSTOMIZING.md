# Customizing devcontainer-wt

This guide walks you through adopting the devcontainer-wt template for your own project. It explains which files you should edit, which files you must not touch, and provides a step-by-step checklist.

## File Map

Every file in the template falls into one of two categories:

| File | Category | What to do |
|---|---|---|
| `.devcontainer/init.sh` | **DO NOT EDIT** | Template engine. Detects worktree context, generates `.env` files, creates Docker network, detects orphans. |
| `.devcontainer/docker-compose.yml` (core sections) | **DO NOT EDIT** | Template engine. Volume mounts, labels, environment, networking. Exception: change the `loadbalancer.server.port` if your app doesn't use port 3000. |
| `.devcontainer/docker-compose.infra.yml` (traefik + networks) | **DO NOT EDIT** | Template engine. Traefik reverse proxy and network definition. |
| `.devcontainer/hooks/post-start.sh` (git symlink fix) | **DO NOT EDIT** | Template engine. The `if [ -f ".git" ]` block that creates the symlink. |
| `.devcontainer/devcontainer.json` (core fields) | **DO NOT EDIT** | Template engine. `name`, `dockerComposeFile`, `service`, `workspaceFolder`, `initializeCommand`, `postStartCommand`, `remoteEnv`. |
| | | |
| `.devcontainer/Dockerfile` | **CUSTOMIZE** | Add your project's system-level dependencies. |
| `.devcontainer/devcontainer.json` (features, extensions) | **CUSTOMIZE** | Add devcontainer features (Node, Python, Go, etc.) and VS Code extensions. |
| `.devcontainer/docker-compose.infra.yml` (your services) | **CUSTOMIZE** | Add infrastructure services (Postgres, Redis, etc.) with `profiles: [infra]`. |
| `.devcontainer/docker-compose.yml` (port, caches) | **CUSTOMIZE** | Change the app port. Add shared build cache volumes. |
| `.devcontainer/hooks/post-start.sh` (project setup) | **CUSTOMIZE** | Add dependency installation, DB initialization, migrations. |
| `.env.app.template` | **CUSTOMIZE** | Define per-worktree environment variables using `${VARIABLE}` placeholders. |
| `.gitignore` | **CUSTOMIZE** | Add your project's ignore patterns. Keep the existing devcontainer-wt entries. |
| `src/`, `package.json` | **REPLACE** | Sample app. Replace entirely with your own application code. |
| `docs/SPEC.md` | **OPTIONAL** | Template design document. Keep for reference or delete. |

## Adoption Checklist

Follow these steps to adopt devcontainer-wt into your project:

### 1. Copy the template files

Copy the `.devcontainer/` directory, `.env.app.template`, `.agents/` (agent skill), and `.gitignore` entries into your project root.

```bash
# From the devcontainer-wt template repo
cp -r .devcontainer/ /path/to/your-project/.devcontainer/
cp .env.app.template /path/to/your-project/
cp -r .agents/ /path/to/your-project/.agents/
# Merge .gitignore entries (don't overwrite your existing .gitignore)
```

Make sure to add these lines to your `.gitignore`:

```
# Generated by devcontainer-wt (per-worktree, not tracked)
.devcontainer/.env
.devcontainer/.env.app
```

### 2. Choose your language/runtime

Edit `.devcontainer/devcontainer.json` and add the appropriate devcontainer feature:

```jsonc
"features": {
  // Pick one (or more):
  "ghcr.io/devcontainers/features/node:1": { "version": "20" },
  "ghcr.io/devcontainers/features/python:1": { "version": "3.12" },
  "ghcr.io/devcontainers/features/go:1": { "version": "1.22" },
  "ghcr.io/devcontainers/features/rust:1": { "version": "latest" }
}
```

### 3. Add system dependencies (if needed)

Edit `.devcontainer/Dockerfile` to install packages not covered by devcontainer features:

```dockerfile
FROM mcr.microsoft.com/devcontainers/base:ubuntu

RUN apt-get update && apt-get install -y \
    postgresql-client \
    redis-tools \
    && rm -rf /var/lib/apt/lists/*
```

### 4. Add infrastructure services

Edit `.devcontainer/docker-compose.infra.yml` to uncomment or add services. Every service **must** have `profiles: [infra]`:

```yaml
  postgres:
    profiles: [infra]
    image: postgres:16
    container_name: "postgres-${PROJECT_NAME}"
    environment:
      POSTGRES_PASSWORD: dev
      POSTGRES_USER: dev
    ports:
      - "${POSTGRES_HOST_PORT:-15432}:5432"
    volumes:
      - pgdata:/var/lib/postgresql/data
    networks:
      - devnet
    restart: unless-stopped
```

Don't forget to add the corresponding volume:

```yaml
volumes:
  pgdata:
    name: "${PROJECT_NAME}-pgdata"
```

### 5. Configure environment variables

Edit `.env.app.template` with your project's per-worktree variables:

```bash
DATABASE_URL=postgres://dev:dev@postgres-${PROJECT_NAME}:5432/${PROJECT_NAME}_${WORKTREE_NAME}
REDIS_URL=redis://redis-${PROJECT_NAME}:6379/0
APP_NAME=${PROJECT_NAME}-${WORKTREE_NAME}
SECRET_KEY=${SECRET_KEY}
```

### 6. Set up post-start hooks

Edit `.devcontainer/hooks/post-start.sh` below the `CUSTOMIZE` marker. Add your dependency installation and database setup:

```bash
# --- Install dependencies ---
npm install

# --- Database initialization ---
PGPASSWORD=dev psql -h "postgres-${PROJECT_NAME}" -U dev -tc \
  "SELECT 1 FROM pg_database WHERE datname = '${PROJECT_NAME}_${WORKTREE_NAME}'" | \
  grep -q 1 || \
  PGPASSWORD=dev createdb -h "postgres-${PROJECT_NAME}" -U dev "${PROJECT_NAME}_${WORKTREE_NAME}"

# --- Migrations ---
npx prisma migrate deploy
```

### 7. Set the app port

If your app listens on a port other than 3000, update the Traefik label in `.devcontainer/docker-compose.yml`:

```yaml
- "traefik.http.services.${PROJECT_NAME}-${WORKTREE_NAME}.loadbalancer.server.port=4000"
```

### 8. Add VS Code extensions

Edit `.devcontainer/devcontainer.json`:

```jsonc
"customizations": {
  "vscode": {
    "extensions": [
      "dbaeumer.vscode-eslint",
      "esbenp.prettier-vscode"
    ]
  }
}
```

### 9. (Optional) Add shared build caches

To share package manager caches across worktrees, uncomment and adapt the volumes section in `.devcontainer/docker-compose.yml`:

```yaml
# Under app.volumes:
- npm-cache:/home/node/.npm

# At the top level:
volumes:
  npm-cache:
    name: "${PROJECT_NAME}-npm-cache"
```

### 10. Test it

```bash
# Open in VS Code and "Reopen in Container"
code .
# Verify your app is reachable at http://{WORKTREE}.{PROJECT}.localhost
```

## Common Scenarios

### Python + PostgreSQL project

```
Dockerfile:     Add postgresql-client
devcontainer.json features:  python:3.12
docker-compose.infra.yml:   Uncomment postgres
.env.app.template:          DATABASE_URL=postgres://...
post-start.sh:              pip install -r requirements.txt + createdb + alembic upgrade head
docker-compose.yml port:    Change to 8000
```

### Go + Redis project

```
Dockerfile:     Add redis-tools (optional, for debugging)
devcontainer.json features:  go:1.22
docker-compose.infra.yml:   Uncomment redis
.env.app.template:          REDIS_URL=redis://...
post-start.sh:              go mod download
```

### Multi-service project (frontend + API)

Define multiple services in `docker-compose.yml` with separate Traefik routes. For example, a frontend at `{WORKTREE}.{PROJECT}.localhost` and an API at `api.{WORKTREE}.{PROJECT}.localhost`. See the agent skill reference at `.agents/skills/devcontainer-wt/references/CUSTOMIZING.md` for a full example.
